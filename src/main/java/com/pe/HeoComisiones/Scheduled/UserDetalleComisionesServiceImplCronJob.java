...
public class UserDetalleComisionesServiceImplCronJob implements UserDetalleComisionesService {
    ...
    public void saveDetalleComisiones() {
        ...
        if (!inversoresConStatusTrue.isEmpty()) {
            // SE RELIZA UN SET PARA EVITAR LA REPETICION DE USUARIOS DE ACUERDO A LOS
            // INVERSORES ACTIVOS
    ...
    private void procesarResultados(Usuarios usuarios) {
        ...
        double ganancia = 0;
        // CADA USUARIO TIENE SU PROPIA COMISION ASIGNADA POR LO QUE NO SE PUEDE
        // ESTANDARIZAR
        ...
        try {
            ...
            // SE SUMA CADA MONTOINVERTIDO DE TODOS LOS INVERSIORES QUE PERTENECEN A UN
            // ASESOR
            montoinvertido = inversorRepository.getInversionesByusuario(usuarios.getId()).stream()
            ...
            if (montoinvertido < 30000) {
                ...
                for (Map.Entry<Double, Double> entry : comisionesPorUsuario.entrySet()) {
                    // VERIFICA HASTA EL BREAK QUE ES DONDE SE QUEDARA CON EL ULTIMO VALOR QUE
                    // REALMENTE DEBERIA TENER
    ...
    private void procesarDetallecomision(Usuarios usuario, double montoinvertido, double comisionCorrrespondiente,
            double ganacia) {
        // COMO LOS USUARIOS QUE ESTAN AQUI SIEMPRE VAN A SER CON EL ESTADO EN TRUE NO
        // SE RELIZARON VALIDACIONES DE EMPTY
        ...
        // SE ENVIA LOS PARAMETROS NECESARIOS PARA ACTUALIZAR LA BASE DE DATOS DE
        // ACUERDO A LOS INVERSORES ACTIVOS
